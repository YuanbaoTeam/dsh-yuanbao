...
import { msgBodyDesensitization } from "../sanitize.js";
...
export class YuanbaoWsClient {
  ...
  private onMessage(raw: WebSocket.RawData): void {
    ...
    if (raw instanceof Buffer) {
      ...
      this.log.warn("ws received non-binary message, ignoring");
      this.log.warn("received non-binary message, ignoring");
      return;
    }

    this.log.info(`ws raw message received: bytes=${binary.length}`);
    const connMsg = decodeConnMsg(binary);
    if (!connMsg?.head) {
      this.log.warn(`ws ConnMsg decode failed: bytes=${binary.length}`);
      this.log.warn("received undecodable ConnMsg");
      return;
    }

    this.log.info(`ws ConnMsg decoded: cmdType=${connMsg.head.cmdType}; cmd=${connMsg.head.cmd}; module=${connMsg.head.module}; msgId=${connMsg.head.msgId}; status=${connMsg.head.status}; data=${connMsg.data?.length ?? 0}`);
  ...
  private onPingResponse(_head: PBConnMsg["head"], data: Uint8Array): void {
    this.heartbeatAckReceived = true;
    this.heartbeatTimeoutCount = 0;
  ...
  private onPush(connMsg: PBConnMsg): void {
    ...
    if (pushMsg && (pushMsg.cmd || pushMsg.module)) {
      ...
      this.log.info(`ws push decoded as PushMsg: cmd=${pushEvent.cmd ?? ''}; module=${pushEvent.module ?? ''}; msgId=${pushEvent.msgId ?? ''}; raw=${rawData?.length ?? 0}; conn=${data?.length ?? 0}`);
    ...
    if (directed && (directed.type || directed.content)) {
      ...
      this.log.info(`ws push decoded as DirectedPush: type=${pushEvent.type ?? ''}; content=${pushEvent.content?.length ?? 0}; msgId=${pushEvent.msgId ?? ''}`);
      this.callbacks.onDispatch?.(pushEvent);
      return;
    }

    // Unrecognized push — pass raw data through
    this.log.warn(`ws push unrecognized, dispatch raw: cmd=${head.cmd}; module=${head.module}; msgId=${head.msgId}; data=${data?.length ?? 0}`);
  ...
  private getReconnectDelay(): number {
    ...
    return delays[index] ?? DEFAULT_RECONNECT_DELAYS[DEFAULT_RECONNECT_DELAYS.length - 1] ?? 60_000;
  }
